                ...
                if (node.Arguments.Count > 0)
                {
                    ...
                    if (sourceType.IsGenericType)
                    {
                        ...
                        if (genericType == typeof(IQueryable<>) || genericType == typeof(DbSet<>))
                        {
                            ...
                            if (typeof(MaemoCompliance.Domain.Common.TenantOwnedEntity).IsAssignableFrom(entityType))
                            {
                                // Add tenant filter before the existing query
                                var parameter = Expression.Parameter(entityType, "e");
                                var tenantIdProperty = Expression.Property(parameter, nameof(MaemoCompliance.Domain.Common.TenantOwnedEntity.TenantId));

...
public static class TenantsEndpoints
{
    public static void MapTenantsEndpoints(this WebApplication app)
    {
        ...
        var createTenantRoute = group.MapPost("/", async (
        ...
        if (!app.Environment.IsDevelopment())
        {
            createTenantRoute.RequireAuthorization("RequireAdmin");
        }

        // GET /api/tenants/{id} - Admin or TenantAdmin only
        var getTenantRoute = group.MapGet("/{id:guid}", async (
            Guid id,
            IMediator mediator,
            CancellationToken cancellationToken) =>
        {
        ...
        if (!app.Environment.IsDevelopment())
        {
            getTenantRoute.RequireAuthorization("RequireAdminOrTenantAdmin");
        }

        // PUT /api/tenants/{id} - Admin or TenantAdmin only
        var updateTenantRoute = group.MapPut("/{id:guid}", async (
            Guid id,
        ...
        if (!app.Environment.IsDevelopment())
        {
            updateTenantRoute.RequireAuthorization("RequireAdminOrTenantAdmin");
        }

        // GET /api/tenants - Admin only
        var getAllTenantsRoute = group.MapGet("/", async (
            IMediator mediator,
            CancellationToken cancellationToken) =>
        {
        ...
        if (!app.Environment.IsDevelopment())
        {
            getAllTenantsRoute.RequireAuthorization("RequireAdmin");
        }

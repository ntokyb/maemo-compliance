using Maemo.Api.Authentication;
using Maemo.Api.Portal;
using Maemo.Api.Public;
using Maemo.Api.Admin;
using Maemo.Api.Development;
using Maemo.Api.Engine;
using Maemo.Api.Middleware;
...
try
{
    Log.Information("Starting Maemo API");
...
"
    ...
    Log.Information("Maemo API running in {Mode} mode", deploymentContext.Mode);
    ...
    if (seedDemoData || isDevelopment)
    {
        ...
        {
            using var scope = app.Services.CreateScope();
            var seeder = scope.ServiceProvider.GetRequiredService<Maemo.Application.Demo.IDemoDataSeeder>();
